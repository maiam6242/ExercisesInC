...
void map_add(Map *map, Hashable *key, Value *value)
{
    ...
    if(map->lists[hash % map-> n] != NULL){
        map->lists[hash % map -> n] = prepend(key, value, map -> lists[hash % map -> n]);
    }
    else
    {
        map->lists[hash % map -> n] = make_node(key, value, NULL);
    }
